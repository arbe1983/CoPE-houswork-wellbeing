...
*---------------------------------------*
	...
	cd "$data"
	foreach file in "prediction_finland" "prediction_germany" "prediction_uk" {
		...
		replace female_pre=1.25 if (n==8|n==18)&female==1
...
replace shortlong_pre=0.95 if (n==12|n==32)&long_term==1&female==0
replace shortlong_pre=1.05 if (n==17|n==37)&long_term==1&female==1
...
ge shortlong_ame=.
replace shortlong_ame=-0.3 if (n==21)&long_term==0&female==0
replace shortlong_ame=0.15 if (n==23)&long_term==0&female==0
replace shortlong_ame=-0.15 if (n==26)&long_term==0&female==1
replace shortlong_ame=0.3 if (n==28)&long_term==0&female==1

replace shortlong_ame=0.7 if (n==31)&long_term==1&female==0
replace shortlong_ame=1.15 if (n==33)&long_term==1&female==0
replace shortlong_ame=0.85 if (n==36)&long_term==1&female==1
replace shortlong_ame=1.3 if (n==38)&long_term==1&female==1	

							
* Short-and-Long in one Figure! *
tw scatter estimate shortlong_ame if estimate_type=="ame" & long_term==0&female==0, msym(O) mc(black) mfc(black) msize(medium) || scatter estimate shortlong_ame if estimate_type=="ame" & long_term==0&female==1, msym(T) mc(black%70) mfc(white) msize(medium) || rcap ub lb shortlong_ame if estimate_type=="ame" & long_term==0&female==0, lc(black%80) || rcap ub lb shortlong_ame if estimate_type=="ame" & long_term==0&female==1, lc(black%50) || scatter estimate shortlong_ame if estimate_type=="ame" & long_term==1&female==0, msym(O) mc(black) mfc(black) msize(medium) || scatter estimate shortlong_ame if estimate_type=="ame" & long_term==1&female==1, msym(T) mc(black%70) mfc(white) msize(medium) || rcap ub lb shortlong_ame if estimate_type=="ame" & long_term==1&female==0, lc(black%80) || rcap ub lb shortlong_ame if estimate_type=="ame" & long_term==1&female==1, lc(black%50) xtit("") ytit("AME (ref.: No change)") yline(0) lcol(gray%80) title("`country'") xlabel(-0.225 "Decrease" 0.225"Increase" 0.775"Decrease" 1.25 "Increase" 0 "{bf:Short-Term}" 1 "{bf:Long-term}") 

gr save "$work\figures/ame_shortlong_`country'.gph", replace
...
*---------------------------------------*
	...
	grc1leg "$work\figures/ame_shortlong_finland.gph" "$work\figures/ame_shortlong_germany.gph" "$work\figures/ame_shortlong_uk.gph", col(3)
	gr save "$work\ame_combined.gph", replace
	
	grc1leg "$work\figures/pred_shortlong_finland.gph" "$work\figures/pred_shortlong_germany.gph" "$work\figures/pred_shortlong_uk.gph", col(3)
	gr save "$work\predictive_combined.gph", replace
	
gr combine "$work\ame_combined.gph" "$work\predictive_combined.gph", col(1)
gr save "$work\pr_ame.gph", replace
